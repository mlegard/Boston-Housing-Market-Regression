...
# rename variable 
colnames(Boston)[colnames(Boston) == 'black'] <- 'B'
...
# # wanted to check to see if one hot encoding vs label encoding made difference, it did not
...
# # get a quick summary to see if encoding method matters, it does not
# summary(model1)
# summary(modelTrain) 
...
summary(modeldisOnlyNum)

# anova of model
anova(numeric_main_effects)
anova(modeldisOnlyNum)
...
# Extract the variables selected by each method
variables_backwarddis <- names(coef(backwarddis))
variables_forwarddis <- names(coef(forwarddis))
variables_stepwisedis <- names(coef(stepwisedis))
...
common_variablesdis <- intersect(intersect(variables_backwarddis, variables_forwarddis), variables_stepwisedis)

# Print the list of common variables
print(common_variables)
print(common_variablesdis)
onlyNumsF <- lm(medv ~ zn + nox + rm + dis + ptratio + B + lstat, data = training1)
# define full models with categorical vars
model_wj_RAD = lm(medv ~ RAD_2 + RAD_3 + RAD_4 + RAD_5 + RAD_6 + RAD_7 + RAD_8 + RAD_9, data = data_cleaned)
model_wj_CHAS = lm(medv ~ chas, data = data_cleaned)
model_with_RAD = lm(medv ~ zn + nox + rm + dis + ptratio + B + lstat + RAD_2 + RAD_3 + RAD_4 + RAD_5 + RAD_6 + RAD_7 + RAD_8 + RAD_9, data = data_cleaned)
model_with_CHAS = lm(medv ~ zn + nox + rm + dis + ptratio + B + lstat + chas, data=data_cleaned)
...
anova(modelWJrad)
anova(model_wj_CHAS)
summary(model_wj_RAD)
summary(model_wj_CHAS)
summary(modelWJrad)
summary(modelWJchas)
anova(modelWJrad)
anova(modelWJchas)

backwardRAD <- step(model_wj_RAD, direction = "backward")
summary(backwardRAD)
anova(numeric_main_effects, model_with_RAD)
anova(numeric_main_effects, model_with_CHAS)
anova(numeric_main_effects, model2)
